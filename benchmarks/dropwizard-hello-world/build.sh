...
"$DIR/mvnw" -f "$DIR/pom.xml" -DskipTests package $maven_options

if [ $# -eq 0 ] || [ $1 != "--skip-nib-generation" ]; then
  "$DIR/mvnw" -f "$DIR/pom.xml" -Pnative -DskipTests package $maven_options
fi
