...
JAR="$DIR/target/helidon-hello-world-jar-with-dependencies.jar"
NIB="$DIR/target/helidon-hello-world-$VERSION.nib"
...
if [ $# -gt 0 ] && [ $1 = "--skip-nib-generation" ]; then
  "$DIR/mvnw" package -f "$DIR/pom.xml"
else
  "$DIR/mvnw" package -Pnative-image -f "$DIR/pom.xml"
fi;
